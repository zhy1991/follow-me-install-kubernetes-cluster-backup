...
+ k8s-dev-master01-54：192.168.31.54
+ k8s-dev-master02-55：192.168.31.55
+ k8s-dev-master03-56：192.168.31.56
+ k8s-dev-node01-57：192.168.31.57
+ k8s-dev-node02-58：192.168.31.58
+ k8s-dev-node03-59：192.168.31.59

注意：
1. 本文档中的 etcd 集群、master 节点使用前三台机器，worker节点使用后三台机器；
1. 需要在**所有机器**上执行本文档的初始化命令；
2. 需要使用 **root 账号**执行这些命令；
3. 如果没有特殊指明，本文档的所有操作**均在 k8s-dev-master01-54 节点上执行**，然后远程分发文件和执行命令；
...
``` bash
hostnamectl set-hostname k8s-dev-master01-54 # 将 k8s-dev-master01-54 替换为当前主机名
```
...
``` bash
cat >> /etc/hosts <<EOF
192.168.31.54 k8s-dev-master01-54 
192.168.31.55 k8s-dev-master02-55 
192.168.31.56 k8s-dev-master03-56 
192.168.31.57 k8s-dev-node01-57 
192.168.31.58 k8s-dev-node02-58 
192.168.31.59 k8s-dev-node03-59 
EOF
```
...
如果没有特殊指明，本文档的所有操作**均在 k8s-dev-master01-54 节点上执行**，然后远程分发文件和执行命令，所以需要添加该节点到其它节点的 ssh 信任关系。

设置 k8s-dev-master01-54 的 root 账户可以无密码登录**所有节点**：

``` bash
ssh-keygen -t rsa
ssh-copy-id root@k8s-dev-master01-54
ssh-copy-id root@k8s-dev-master02-55
ssh-copy-id root@k8s-dev-master03-56
ssh-copy-id root@k8s-dev-node01-57
ssh-copy-id root@k8s-dev-node02-58
ssh-copy-id root@k8s-dev-node03-59
```
