...
注意：如果没有特殊指明，本文档的所有操作**均在 k8s-dev-master01-54 节点上执行**，然后远程分发文件和执行命令。
...
5xwf9z.2kgjmjdrr9d58xez   23h       2019-10-09T14:34:12+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-master03-56
au3vbl.6n1j3h6cj2nbht95   23h       2019-10-09T14:34:13+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-node02-58
jopvj8.p35zcfpestn2f9zs   23h       2019-10-09T14:34:10+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-master01-54
kip5k3.i7983fegr2kfwdh1   23h       2019-10-09T14:34:13+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-node03-59
llm7y3.e0gbhaotr1p4y89t   23h       2019-10-09T14:34:12+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-node01-57
tq4xmh.rfejph0koefhd8yv   23h       2019-10-09T14:34:11+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-dev-master02-55
```
...
bootstrap-token-5xwf9z                           bootstrap.kubernetes.io/token         7      84s
bootstrap-token-au3vbl                           bootstrap.kubernetes.io/token         7      83s
bootstrap-token-jopvj8                           bootstrap.kubernetes.io/token         7      86s
bootstrap-token-kip5k3                           bootstrap.kubernetes.io/token         7      83s
bootstrap-token-llm7y3                           bootstrap.kubernetes.io/token         7      84s
bootstrap-token-tq4xmh                           bootstrap.kubernetes.io/token         7      85s
```

## 分发 bootstrap kubeconfig 文件到所有 worker 节点 (注意：此文件中有kube-apiservice地址信息)
...
## 为各节点创建和分发 kubelet 配置文件：(注意：此文件中有kube-apiservice地址信息)
...
``` bash
$ sudo journalctl -f -u kubelet -a |grep -A 2 'certificatesigningrequests'
...
192.168.31.54   Ready    <none>   30m     v1.14.2
192.168.31.55   Ready    <none>   30m     v1.14.2
192.168.31.56   Ready    <none>   30m     v1.14.2
192.168.31.57   Ready    <none>   5s      v1.14.2
192.168.31.58   Ready    <none>   91s     v1.14.2
192.168.31.59   Ready    <none>   3m27s   v1.14.2
```

## 为每个节点设置角色
``` bash
kubectl label nodes 192.168.31.54 node-role.kubernetes.io/master=
kubectl label nodes 192.168.31.55 node-role.kubernetes.io/master=
kubectl label nodes 192.168.31.56 node-role.kubernetes.io/master=
kubectl label nodes 192.168.31.57 node-role.kubernetes.io/node=
kubectl label nodes 192.168.31.58 node-role.kubernetes.io/node=
kubectl label nodes 192.168.31.59 node-role.kubernetes.io/node=

$ kubectl get nodes
NAME            STATUS   ROLES    AGE   VERSION
192.168.31.54   Ready    master   43m   v1.14.2
192.168.31.55   Ready    master   43m   v1.14.2
192.168.31.56   Ready    master   43m   v1.14.2
192.168.31.57   Ready    node     12m   v1.14.2
192.168.31.58   Ready    node     14m   v1.14.2
192.168.31.59   Ready    node     16m   v1.14.2
```
...
## 批量
$ for i in `kubectl get csr | awk -F" " '{print $1}'`;do kubectl certificate approve $i;done
...
tcp        0      0 192.168.31.54:10248    0.0.0.0:*               LISTEN      129697/kubelet
tcp        0      0 192.168.31.54:10250    0.0.0.0:*               LISTEN      129697/kubelet
```
...
``` bash
$ # 权限不足的证书；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://192.168.31.54:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

$ # 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://192.168.31.54:10250/metrics|head
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://192.168.31.54:10250/metrics|head
# HELP apiserver_audit_event_total Counter of audit events generated and sent to the audit backend.
# TYPE apiserver_audit_event_total counter
apiserver_audit_event_total 0
# HELP apiserver_audit_requests_rejected_total Counter of apiserver requests rejected due to an error in audit logging backend.
...
浏览器访问 https://192.168.31.54:10250/metrics 和 https://192.168.31.54:10250/metrics/cadvisor 分别返回 kubelet 和 cadvisor 的 metrics。
...
$ # 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
$ source /opt/k8s/bin/environment.sh
$ sudo curl -sSL --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem ${KUBE_APISERVER}/api/v1/nodes/192.168.31.54/proxy/configz | jq \
  '.kubeletconfig|.kind="KubeletConfiguration"|.apiVersion="kubelet.config.k8s.io/v1beta1"'
{
  ...
  "enableContentionProfiling": true,
  "healthzPort": 10248,
  "healthzBindAddress": "192.168.31.54",
  "oomScoreAdj": -999,
